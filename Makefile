# VGI easter worker (V) — requires: v, Apache Arrow (brew install apache-arrow)
ROOT := $(abspath .)
...
shim: vgi_v/c/libvgi_ipc.dylib

vgi_v/c/libvgi_ipc.dylib: vgi_v/c/vgi_ipc.cpp vgi_v/c/vgi_ipc.h
...
bin/vgi_smoke_client: vgi_v/c/vgi_smoke_client.cpp vgi_v/c/libvgi_ipc.dylib
	@mkdir -p bin
	c++ -std=c++20 -O2 vgi_v/c/vgi_smoke_client.cpp -o $@ -Ivgi_v/c -Lvgi_v/c -lvgi_ipc $(ARROW_PKG) \
	  -Wl,-rpath,@loader_path/../vgi_v/c

bin/vgi_error_smoke_client: vgi_v/c/vgi_error_smoke_client.cpp vgi_v/c/libvgi_ipc.dylib
	@mkdir -p bin
...
clean:
	rm -f $(EASTER_WORKER) $(SMOKE_CLIENT) vgi_v/c/libvgi_ipc.dylib
